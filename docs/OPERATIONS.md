...
2. **Backfill scoring (Atlas + Beacon)**  
   ...
   **By default**, Celery Beat runs the same **incomplete-pair** logic on a conservative schedule (parcels missing entitlement or strategic — every **12 hours** UTC, 50 parcels per run by default — see **`SCHEDULED_ENQUEUE_*`** in `deploy/env.production.example`). Restart **worker + beat** after changing those variables.
