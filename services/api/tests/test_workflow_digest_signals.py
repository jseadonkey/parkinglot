...
def test_scheduled_enqueue_unscored_defaults() -> None:
    """Periodic backlog drain stays on, but defaults to small, infrequent batches."""
    s = Settings()
    assert s.scheduled_enqueue_unscored_enabled is True
    assert s.scheduled_enqueue_unscored_limit == 50
    assert s.scheduled_enqueue_unscored_crontab_hour == "*/12"
